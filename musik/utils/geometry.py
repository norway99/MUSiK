...
class Transform:
    ...
    def __post_init__(self):
        if self.from_matrix:
            if isinstance(self.rotation, np.ndarray):
                self.rotation = scipy.spatial.transform.Rotation.from_matrix(self.rotation)
            else:
                raise Exception("Please supply the rotation in matrix format")
        elif self.about_axis:
            if self.rotation == (0, 0, 0):
                self.rotation = scipy.spatial.transform.Rotation.from_matrix(np.eye(3))
            else:
                self.rotation = scipy.spatial.transform.Rotation.from_rotvec(self.rotation)
        elif self.intrinsic:
            if self.ordering is not None:
                self.rotation = scipy.spatial.transform.Rotation.from_euler(
                    self.ordering, self.rotation, degrees=False
                )
            else:
                self.rotation = scipy.spatial.transform.Rotation.from_euler(
                    "ZYX", self.rotation, degrees=False
                )  # intrinsic
        else:
            if self.ordering is not None:
                self.rotation = scipy.spatial.transform.Rotation.from_euler(
    ...
    def make_from_heading_vector(cls, heading_vector, trans):
        e = np.array([1, 0, 0])
        rej = (heading_vector - np.dot(e, heading_vector) * e) / np.linalg.norm(
            heading_vector - np.dot(e, heading_vector) * e
        )
        ...
        g = np.array(
            [
                [
                    np.dot(e, heading_vector),
                    -1 * np.linalg.norm(np.cross(e, heading_vector)),
                    0,
                ],
                [
                    np.linalg.norm(np.cross(e, heading_vector)),
                    np.dot(e, heading_vector),
                    0,
                ],
                [0, 0, 1],
    ...
    def __get_matrix(
        ...
        if inverse:
            ...
            transform[:-1, -1] = (
                -self.rotation.apply(self.translation, inverse=True) / scale
            )
    ...
    def __greedy_padding(self, array, scale, padwith):
        ...
        min_x = np.floor(
            np.min(np.concatenate((inverse_vert[:, 0], vert[:, 0]), axis=0))
        )
        max_x = np.ceil(
            np.max(np.concatenate((inverse_vert[:, 0], vert[:, 0]), axis=0))
        )
        min_y = np.floor(
            np.min(np.concatenate((inverse_vert[:, 1], vert[:, 1]), axis=0))
        )
        max_y = np.ceil(
            np.max(np.concatenate((inverse_vert[:, 1], vert[:, 1]), axis=0))
        )
        min_z = np.floor(
            np.min(np.concatenate((inverse_vert[:, 2], vert[:, 2]), axis=0))
        )
        max_z = np.ceil(
            np.max(np.concatenate((inverse_vert[:, 2], vert[:, 2]), axis=0))
        )
            ...
            for i in range(array.shape[0]):
                padded.append(
                    np.pad(
                        ...
                        (x_pad, y_pad, z_pad),
                        mode="constant",
                        constant_values=(padwith[i],),
                    )
                )
            padded = np.stack(padded, axis=0)
        else:
            print(
                "padding currently only supported for transformations of 3 dimensional single or multichannel arrays"
            )
            return 0
    ...
    def apply_to_array(
        self, array, scale=1, padwith=0, inverse=False, order=0, mode="nearest"
    ):
        ...
        if len(array.shape) > 3:
            ...
            translated = scipy.ndimage.shift(
                array, translation, order=order, mode="constant", cval=padwith
            )
        ...
        if len(array.shape) > 3:
            ...
            tarray = scipy.ndimage.affine_transform(
                translated,
                transform[:-1, :-1],
                order=order,
                mode="constant",
    ...
    def apply_to_point(self, v, inverse=False, scale=1):
        if inverse:
            return np.matmul(
                self.__get_matrix(inverse=True, scale=scale),
                np.array([v[0], v[1], v[2], 1]).T,
            )[:3]
        return np.matmul(
            self.__get_matrix(scale=scale), np.array([v[0], v[1], v[2], 1]).T
        )[:3]
    ...
    def apply_to_points(self, v, inverse=False, scale=1):
        if inverse:
            return np.einsum(
                "ij,mj->mi",
                self.__get_matrix(inverse=True, scale=scale),
                np.array([v[:, 0], v[:, 1], v[:, 2], np.ones_like(v[:, 0])]).T,
            )[:, :3]
        return np.einsum(
            "ij,mj->mi",
            self.__get_matrix(scale=scale),
            np.array([v[:, 0], v[:, 1], v[:, 2], np.ones_like(v[:, 0])]).T,
        )[:, :3]
    ...
    def save(self):
        return {
            "rotation": tuple(self.rotation.as_euler("ZYX", degrees=False)),
    ...
    def __copy__(
        ...
        return self.__class__(
            rotation=self.rotation.as_euler("ZYX", degrees=False),
...
def create_sphere(centroid, radius, voxel_dims, matrix_dims):
    ...
    centroid = (
        int(centroid[0] / voxel_dims[0] + matrix_dims[0] / 2),
        int(centroid[1] / voxel_dims[1] + matrix_dims[1] / 2),
        int(centroid[2] / voxel_dims[2] + matrix_dims[2] / 2),
    )

    x, y, z = np.mgrid[
        0 : matrix_dims[0] : 1, 0 : matrix_dims[1] : 1, 0 : matrix_dims[2] : 1
    ]
    r = np.sqrt(
...
def generate_pose_spherical(
    ...
    theta_p = (
        theta  # - np.pi  # assign direction to be towards origin with some variance
    )

    if pitch_fraction == 0:
        phi_p = (
            -phi + np.pi / 2
        )  # assign direction to be towards origin with some variance
    else:
        phi_p = (
            -phi + np.pi / 2
        )  # assign direction to be towards origin with some variance
...
def generate_pose_cylindrical(r_mean=5e-3, r_std=0, view_std=0, z_range=None, rng=None):
    ...
    theta_p = np.mod(
        (randn() * view_std - theta + 2 * np.pi), 2 * np.pi
    )  # assign direction to be towards origin with some variance
    phi_p = np.mod(
        (randn() * view_std - theta + 2 * np.pi), 2 * np.pi
    )  # assign direction to be towards origin with some variance
