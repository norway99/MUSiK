...
class TransducerSet:
    ...
    def save(self, save_file):
        ...
        for t, p in zip(
            self.transducers, self.poses
        ):  # consider getting the dictionary obj for transducer and pose before zipping and saving
    ...
    def generate_extrinsics(
        ...
        if shape == "constrained":
            if tissue_mask is None or target is None or attenuation_threshold is None:
                raise Exception(
                    "In order to use option 'constrained,' \
            ...
            for i in range(self.n_transducers):
                orientation, position = geometry.generate_pose_spherical(
                    **extrinsics_kwargs, rng=self.rng
                )
            ...
            for i in range(self.n_transducers):
                orientation, position = geometry.generate_pose_cylindrical(
                    **extrinsics_kwargs, rng=self.rng
                )
                transform = geometry.Transform(orientation, position, intrinsic=True)
                self.assign_pose(i, transform)
            ...
            for i in range(self.n_transducers):
                orientation, position = geometry.generate_random(
                    **extrinsics_kwargs, rng=self.rng
                )
                transform = geometry.Transform(orientation, position, intrinsic=True)
                self.assign_pose(i, transform)
    ...
    def transmit_poses(
        ...
        return [
            pose
            for pose, transducer in zip(self.poses, self.transducers)
            if transducer.transmit
        ]
    ...
    def remove_pose(self, index):
        print(
            "removing poses without removing transducers is not supported, either overwrite the pose (assign_pose) or remove the transducer (remove_transducer)"
        )
        return 0
    ...
    def place_on_mesh(
        ...
        if vertex_id is not None:
            ...
            if triangle_id is not None:
                ...
                triangle_vertex_coords = np.vstack(
                    surface_mesh.vertex.positions[triangle_vertices[0]].numpy(),
    ...
    ):  # remove the transducer at position i
        if label is None and index is None:
            raise Exception(
                "Please supply a valid label or index to remove a transducer"
            )
    ...
    def plot_transducer_coords(
        ...
        for i in range(len(self.transducers)):
            self.transducers[i].plot_sensor_coords(
                ax=ax, transform=self.poses[i], color=cmap(i / len(self.transducers))
            )
        if phantom_coords is not None:
            phantom_coords = phantom_coords - np.array(
                [
                    0,
                    0,
    ...
    def plot_transducers(self):
        ...
        for i in range(len(self.transducers)):
            coords.append(
                self.poses[i].apply_to_points(self.transducers[i].sensor_coords)
            )
    ...
    def plot_transducer_fovs(
        self, ax=None, save=False, save_path=None, scale=0.1, view=None
    ):
        if ax is None:
            fig = plt.figure()
            ax = fig.add_subplot(projection="3d")
        ...
        cmap = plt.get_cmap("tab20b")
        for i in range(len(self.transducers)):
            self.transducers[i].plot_fov(
                ax=ax,
                transform=self.poses[i],
                length=0.02,
                color=cmap(i / len(self.transducers)),
            )
        if ax is None:
            if save:
                plt.savefig(save_path)
